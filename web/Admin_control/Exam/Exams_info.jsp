...
<body>
<jsp:useBean id="exam_bean" class="com.sql.SQL" scope="page"/>
<%
    Map<String,String> Tea_name=new HashMap<String,String>();
    ResultSet Tea_rs=exam_bean.executeQuery("select * from Teacher");
    try {
        while(Tea_rs.next())
        {
    ...
    ResultSet Ma_rs=exam_bean.executeQuery("select distinct major_id,major_name from major");
    try {
        while(Ma_rs.next())
        {
...
<table border="1" bgcolor="#bc8f8f">
    ...
    <%
        ...
        ResultSet rs=exam_bean.executeQuery(sql);
        try {
            while(rs.next())
            {
                out.println("<tr>");
                ...
                if(rs.getString("major_id").equals("null"))
                    out.println("<td>暂无</td>");
                else
    ...
    <input type="button" name="ADD" value="新增" onclick="location.href='add_exam.jsp'">
</body>
</html>
